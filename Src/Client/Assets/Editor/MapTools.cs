...
public class MapTools :MonoBehaviour{
    ...
    public static void ExportTeleporters()
    {
        ...
        foreach (var map in DataManager.Instance.Maps)
        {
            ...
            foreach (var teleporter in teleporters)//遍历传送点
            {
                ...
                {
                    EditorUtility.DisplayDialog("错误",string.Format("地图：{0} 中配置的 Teleporter :[{1}]  MapID:{2} 错误", map.Value.Resource, teleporter.ID,def.MapID),"错误");
                    return;
                }
