...
public class UIEquipItem : MonoBehaviour,IPointerClickHandler
{
    ...
    internal void SetEquipItem(int idx, Item item, UICharEquip owner, bool equiped)
    {
        ...
        if (this.lexel != null) this.lexel.text = "Lv." + this.item.Define.Level.ToString();
    ...
    {
        if(this.isEquiped)//装备中
        {
            UnEquip();
        }
        else
        {
            if (this.selected)//双击执行
            {
                DoEquip();
                this.Selected = false;
            }
    ...
    private void DoEquip()
    {
        ...
        msg.OnYes = () =>
         {
             var oldEquip = EquipManager.Instance.GetEquip(item.EquipInfo.Slot);//获得当前槽位Item
             if (oldEquip != null)//原来有装备
             {
