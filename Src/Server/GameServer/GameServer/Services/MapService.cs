...
namespace GameServer.Services
{
    class MapService : Singleton<MapService>
    {
        ...
        private void OnMapEntitySync(NetConnection<NetSession> sender, MapEntitySyncRequest request)
        {
            ...
            Log.InfoFormat("OnMapEntitySync:characterID:{0}:{1} Entity.id:{2} Evt:{3} Entity:{4}",character.entityId,character.Info.Name,request.entitySync.Id,request.entitySync.Event,request.entitySync.Entity.String());
        ...
        public void SendEntityUpdate(NetConnection<NetSession> conn, NEntitySync entity)
        {
            ...
            Log.InfoFormat("SendEntityUpdate:entity.id:{0}",entity.Id);//entityid
        ...
        private void OnMapTeleport(NetConnection<NetSession> sender, MapTeleportRequest request)
        {
            ...
            if (!DataManager.Instance.Teleporters.ContainsKey(request.teleporterId))//校验入口
            {
            ...
            if (source.LinkTo == 0 || !DataManager.Instance.Teleporters.ContainsKey(source.LinkTo))//校验出口
            {
