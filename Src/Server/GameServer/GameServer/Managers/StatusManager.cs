...
namespace GameServer.Managers
{
    class StatusManager
    {
        ...
        public StatusManager(Character owner)//构造函数，创建一个新的列表维护状态
        {
            this.Owner = owner;
            this.Status = new List<NStatus>();
        }

        public void AddStatus(StatusType type, int id, int value, StatusAction action)//共有Add方法
        {
        ...
        public void AddGoldChange(int goldDelta)//封装的金币增加方法
        {
            if (goldDelta > 0)
            {
        ...
        public void AddItemChange(int id, int count, StatusAction action)//封装的ItemAdd方法
        {
